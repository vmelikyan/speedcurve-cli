...
test("SpeedCurve.urls.update(script)", async () => {
	...
	const response = await SpeedCurve.urls.update(SPEEDCURVE_API_KEY, 8081, { script: "navigate	https://speedcurve.com/" })

	expect(response.status).toEqual("success")
	expect(request.put.mock.calls[0][0].form.script).toEqual("navigate	https://speedcurve.com/")
})
